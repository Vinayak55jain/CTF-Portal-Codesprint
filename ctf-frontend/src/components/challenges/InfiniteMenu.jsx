...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0, textureUpdateCallback) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #initTexture() {
    ...
    // Notify that texture is ready to be updated
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  // Render folders to texture atlas
  useEffect(() => {
    const renderFoldersToTexture = () => {
      const hiddenCanvas = hiddenCanvasRef.current;
      if (!hiddenCanvas) return;

      const itemsToRender = items.length ? items : defaultItems;
      const itemCount = Math.max(1, itemsToRender.length);
      const atlasSize = Math.ceil(Math.sqrt(itemCount));
      const cellSize = 512;

      hiddenCanvas.width = atlasSize * cellSize;
      hiddenCanvas.height = atlasSize * cellSize;

      const ctx = hiddenCanvas.getContext('2d');
      ctx.clearRect(0, 0, hiddenCanvas.width, hiddenCanvas.height);

      // Create a temporary container for rendering React components
      ...
      Promise.all(promises).then(() => {
        ...
        if (sketchRef.current) {
          sketchRef.current.updateTexture(hiddenCanvas);
        }
      });
    };

    // Delay to ensure sketch is initialized
    const timer = setTimeout(renderFoldersToTexture, 200);
